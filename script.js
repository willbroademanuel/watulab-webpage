...
/* =========================================================================
   11. SMART ROUTING — RETURNING USER BANNER
   =========================================================================
   Tracks which product apps a visitor has opened (via "Fungua App" buttons
   on the homepage) and, on return visits, shows a shortcut banner so they
   don't have to scroll back through the marketing funnel.
*/
(function initSmartRouting() {
    ...
    const APP_REGISTRY = {
        studio: { name: 'Studio AI', url: 'https://studio.watulab.com?ref=returning', icon: 'fa-wand-magic-sparkles' },
        studypal: { name: 'StudyPal', url: 'https://studypal.watulab.com?ref=returning', icon: 'fa-graduation-cap' },
        scope: { name: 'ScopeAI', url: 'https://scope.watulab.com?ref=returning', icon: 'fa-diagram-project' }
    };

    const STORAGE_KEY = 'watu_app_visits';
    const DISMISS_KEY = 'watu_banner_dismissed';
    ...
    function recordVisit(appKey) {
        try {
            const visits = getVisits();
            const existing = visits[appKey] || { count: 0 };
            visits[appKey] = {
                lastVisit: Date.now(),
            ...
            // Private browsing, storage disabled, or quota exceeded — safe to ignore.
        }
    }

    // Track clicks on "Fungua App" buttons (data-app attribute) so the next
    // homepage visit can surface a shortcut.
    document.addEventListener('click', function (e) {
        const anchor = e.target.closest('a[data-app]');
        if (anchor && APP_REGISTRY[anchor.dataset.app]) {
            recordVisit(anchor.dataset.app);
        }
    });

    // Don't rebuild the banner if the visitor already dismissed it this session.
    try {
        if (sessionStorage.getItem(DISMISS_KEY) === '1') return;
    } catch (error) {
        // If sessionStorage is unavailable, fall through and show the banner anyway.
    }

    const visits = getVisits();
    const visitedApps = Object.keys(visits).filter(function (key) {
        return APP_REGISTRY.hasOwnProperty(key);
    });

    if (visitedApps.length === 0) return; // First-time visitor — nothing to show.

    visitedApps.sort(function (a, b) {
        return visits[b].lastVisit - visits[a].lastVisit;
    });

    const shortcuts = visitedApps.slice(0, 2).map(function (key) {
        const app = APP_REGISTRY[key];
        return '<a href="' + app.url + '" target="_blank" rel="noopener noreferrer" data-app="' + key + '" ' +
            'class="inline-flex items-center gap-2 px-4 py-2 bg-white text-black font-bold text-sm rounded-full hover:bg-gray-200 transition-all" ' +
            'aria-label="Fungua ' + app.name + '">' +
            '<i class="fas ' + app.icon + ' text-xs"></i> ' + app.name +
            '</a>';
    }).join('');

    const banner = document.createElement('div');
    banner.id = 'returning-user-banner';
    banner.setAttribute('role', 'complementary');
    banner.setAttribute('aria-label', 'Rudi kwenye programu yako');
    banner.className = 'fixed bottom-0 left-0 right-0 z-50 bg-white/5 backdrop-blur-xl border-t border-white/10 px-4 py-3 transform translate-y-full transition-transform duration-500';
    banner.innerHTML =
        '<div class="max-w-7xl mx-auto flex items-center justify-between gap-4">' +
        '<span class="text-gray-300 text-sm font-medium hidden sm:inline">Karibu tena!</span>' +
        '<div class="flex items-center gap-3 flex-wrap">' + shortcuts + '</div>' +
        '<button id="dismiss-banner" class="text-gray-500 hover:text-white transition-colors p-1" aria-label="Funga">' +
    ...
    if (dismissBtn) {
        dismissBtn.addEventListener('click', function () {
            ...
            try {
                ...
                // Ignore — banner simply reappears next reload if storage is unavailable.
            }
        });
    }
})();
